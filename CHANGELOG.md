...
* **Native Telecom Core Ringing Restoration:** Deleted the `android.telecom.IN_CALL_SERVICE_RINGING` override from the manifest. By returning ringtone and vibration responsibilities to the Native Android OS, we completely bypassed the issue of background Notifications silently failing on Android 13/14 when notification permissions are not fully configured.
* **Default Dialer Background Activity Launch Bypass:** Leveraged the official Android 10+ Background Service Exemption for Default Dialers, explicitly spawning `IncomingCallActivity` directly to securely hijack the lockscreen without notification proxies.
